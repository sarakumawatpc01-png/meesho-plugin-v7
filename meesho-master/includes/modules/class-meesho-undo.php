...
wp_schedule_event( time(), 'weekly', 'meesho_purge_old_snapshots' );
}
}

public function log( $action_type, $post_id, $old_value, $new_value, $source = 'manual' ) {
$logger = new MM_Logger();
return $logger->log_before_change( $action_type, 'post', $post_id, $old_value, $new_value, 0, $source );
}
...
( new MM_Logger() )->log_before_change( 'seo_undo', $row->target_type, $row->target_id, $row->new_value, $row->old_value, (int) $row->suggestion_id, 'manual', 'undo' );
return true;
}
